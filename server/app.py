...
APP_VERSION = "v2025-10-20-final+list"
...
def requests_route():
	...
	if img_b64:
		...
		try:
			...
			with open(save_path, "wb") as f:
				f.write(img_bytes)
			# ← フロントから参照する用は常に "uploads/<fname>" で統一
			public_rel_path = f"uploads/{fname}"
	...
	if public_rel_path:
		rec["server_saved_image"] = public_rel_path

	# 保存
	arr.append(rec)
	data_file.write_text(json.dumps(arr, ensure_ascii=False, indent=2), encoding="utf-8")

	return jsonify(ok=True, id=rec["id"], saved=(public_rel_path or None)), 201
